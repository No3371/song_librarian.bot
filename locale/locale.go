...
func SetLanguage (lang Locale) {
	...
	case TW:
		...
		DETECTED_REDIRECTED = "▶️ **%s**\n**( º﹃º｀ )** 猜測為 *%s*，但最近有轉發過，預設 ❌*不轉發*。%d 秒內可對此訊息投票修改分類：🇴 原創 / 🇨 翻唱 / 🇸 歌回 / ❌ 不轉發"
		DETECTED_PRE_TYPED = "▶️ **%s**\n**(\\*゜ω゜)ゞ** 分享者指定為 *%s*，%d 秒後自動轉發。可對此訊息投票修改分類：🇴 原創 / 🇨 翻唱 / 🇸 歌回 / ❌ 不轉發"
		DETECTED_PRE_TYPED_AGREED = "▶️ **%s**\n**(\\*゜ω゜)ゞ** 分享者指定為 *%s*（而且我也這麼認為！）%d 秒後自動轉發。可對此訊息投票修改分類：🇴 原創 / 🇨 翻唱 / 🇸 歌回 / ❌ 不轉發"
		C_DESC = "channel"
