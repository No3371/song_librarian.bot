...
var regexStream_s2 = regexp2.MustCompile(`sing(?!ing)|singing\s?stream|songs?\s?stream|sings?\s?stream|歌枠(?!切り抜き)|歌回|歌回|うたう|歌う|歌い.?ま.?す|歌配信|歌練|弾き語り|お歌|歌ったり|karaoke|(うた|歌)?ゲリラ.*?(うた|歌)|(うた|歌)ゲリラ.*?(うた|歌)?`, regexp2.IgnoreCase)
...
var regexBadForOriginal = regexp2.MustCompile(`cover.+original\s?mv|original\s?mv.+cover|cover.+mv|mv.+cover`, regexp2.IgnoreCase)
